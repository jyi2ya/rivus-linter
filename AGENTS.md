...
**违规（violation）**分四种：函数调用越权（`Call`）、静态变量引用越权（`StaticRef`）、stub 宏（`StubMacro`）和空函数体（`EmptyFn`）。

- 调用越权：函数调用了自身能力不允许的函数
- 静态引用越权：函数引用了 `static` 或 `thread_local!` 变量但缺少相应的能力（`static` 不可变引用需要 `S`，`static mut` 引用需要 `S` + `U`，`thread_local!` 引用需要 `S` + `T`）
- Stub 宏：函数体包含 `todo!()` 或 `unimplemented!()`——未实现的存根，调用必崩
- 空函数体：函数体无任何逻辑（仅含 `debug_assert!`）——静默不做任何事
...
| `TodoCommentWarning` | 代码中包含 `// TODO` 或 `// FIXME` 注释——待办标记（字符串扫描，不依赖 AST） |
| `UntestedGoodFnWarning` | 好函数（能力 ≤ ABM）未被任何测试调用——应有测试覆盖 |
| `ErrorSwallowWarning` | 调用 `.ok()` 或 `.unwrap_or_default()` 静默吞掉错误——应显式处理 |
| `CatchUnwindWarning` | 使用 `catch_unwind`——应修 panic 源头而非捕获（参见"治根不治标"） |
| `CatchAllErrorVariantWarning` | 错误枚举含 `Unknown`/`Other` 兜底变体——应穷举所有失败模式 |
| `MissingTestOutputWarning` | `#[test]` 函数缺少对应的 `test_out/{name}.out` 快照文件（仅在 `test_out/` 目录存在时检查） |
...
1. **MIR 不执行额外 lint 检查**：MIR 检查仅做能力合规性检查（违规 + 推断提示），不执行上述 25 种警告检查（被禁导入、测试命名、私有函数、缺文档、通配导入等）。这些 lint 依赖 syn 的 AST 分析，须通过 `rivus-linter check` 运行。
...
use rivus_linter::{
    // 能力类型
    Capability, CapabilitySet, rvs_parse_function,
    // 函数提取
    ...
    CheckOutput, CheckError, Violation, ViolationKind, Warning, InferenceWarning, InferenceKind,
    BannedImportWarning, BorrowedParamWarning, CatchAllErrorVariantWarning, CatchUnwindWarning,
    ConsumedArgOnErrorWarning, DeadCodeWarning, DenyWarningsWarning, DerefPolymorphismWarning,
    DuplicateTestWarning, ErrorSwallowWarning, IntoImplWarning, MissingAllowWarning,
    MissingAssertWarning, MissingDebugWarning, MissingDocWarning, MissingPanicsDocWarning,
    MissingSafetyDocWarning, MissingTestOutputWarning, NonRvsFnWarning, ReflectionUsageWarning,
    TestNameFormatWarning, TodoCommentWarning, UntestedGoodFnWarning, WildcardImportWarning,
    MirCheckError,
    // 报告
...
| `rvs_check_mir_dir_BIM(mir_dir, path, capsmap)` | 检查 MIR 目录下的 `.mir` 文件的能力合规性 |
| `rvs_check_mir_path_BIMPS(path, capsmap)` | 编译项目到 MIR 并检查（完整流程） |
...
| 类型 | 用途 |
|------|------|
| `CheckError` | `rvs_check_path_BI` 的 I/O 错误 |
| `ExtractError` | 所有 `rvs_extract_*` 函数的解析错误 |
...
| `rvs_extract_non_rvs_fns(source)` | 提取所有函数（pub 和非 pub），用于检查 rvs_ 前缀 |
...
| `rvs_extract_stub_macros(source)` | 提取 `todo!()`/`unimplemented!()` 存根宏使用 |
| `rvs_extract_empty_fns(source)` | 提取空函数体（含仅 `debug_assert!` 的函数） |
| `rvs_extract_todo_comments(source)` | 提取 `// TODO`/`// FIXME` 注释（字符串扫描） |
| `rvs_extract_test_call_names(source)` | 提取 `#[test]` 函数体内调用的函数名 |
| `rvs_extract_error_swallows(source)` | 提取 `.ok()` / `.unwrap_or_default()` 静默吞错调用 |
| `rvs_extract_catch_unwind(source)` | 提取 `catch_unwind` 使用 |
| `rvs_extract_catch_all_error_variants(source)` | 提取错误枚举中的 `Unknown`/`Other` 兜底变体 |
