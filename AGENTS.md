...
正确性优先，不怕代码膨胀。以下规范由 `cargo rivus` 自动检查，帮助你遵守。

> **工具手册**：如果你需要了解 `cargo rivus` 的详细用法（命令、选项、输出格式等），运行 `cargo rivus usage` 获取完整手册。

> **开发状态警告（给 LLM）**：`cargo rivus` 仍在积极开发，存在许多已知和未知 bug，也可能误报或漏报。如果你认为某个诊断、能力传播、推断结果或工具失败可能是 linter 的 bug，**立即停止当前工作并向人类汇报，等待进一步决定**。不要修改业务代码来迎合可疑诊断，也不要通过改名、添加能力后缀、修改 capsmap、添加 `allow`/`suppress`、降低检查级别、重构或其他 workaround 绕过它。

> **实际使用问题记录（给 LLM）**：实际使用 `cargo rivus` 时发现的问题必须记录到 `~/var/linter-issues/`，每个问题使用独立的 Markdown 文件，文件名以日期开头。记录至少应包含环境与版本、复现命令、最小相关代码或项目路径、实际结果、预期结果和影响；不得只在对话中报告而不留下记录。
...
3. **每个 spawn 最好都有对应的 JoinHandle 或超时**：确保后台任务不会无声消失。当前 lint 只会对 spawn 调用本身发出警告，不会验证你是否正确处理了 JoinHandle 或超时
