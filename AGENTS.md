...
| `StubWarning` | 函数体包含 `todo!()` 或 `unimplemented!()`——未实现的存根 |
| `EmptyFnWarning` | 函数体为空或仅含 `debug_assert!`——无实际逻辑 |
...
1. **MIR 不执行额外 lint 检查**：MIR 检查仅做能力合规性检查（违规 + 推断提示），不执行上述 20 种警告检查（被禁导入、测试命名、私有函数、缺文档、通配导入等）。这些 lint 依赖 syn 的 AST 分析，须通过 `rivus-linter check` 运行。
