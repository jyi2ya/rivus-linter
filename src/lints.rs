...
impl<'tcx> LateLintPass<'tcx> for RivusLintPass {
        ...
        match &item.kind {
            ItemKind::Fn {
                ...
                if !name.starts_with("rvs_") {
                    self.rvs_collect_callgraph_for_item_M(cx, item.hir_id(), sig, body);
                }
                if is_test {
                    self.test_names
                ...
                rvs_check_missing_safety_doc_S(cx, item.hir_id(), item.span, &sig.header.safety);
    ...
    fn check_impl_item(&mut self, cx: &LateContext<'tcx>, impl_item: &'tcx ImplItem<'tcx>) {
        if let ImplItemKind::Fn(sig, body_id) = &impl_item.kind {
            ...
            if !is_trait_impl {
                ...
                if !name.starts_with("rvs_") {
                    self.rvs_collect_callgraph_for_item_M(cx, impl_item.hir_id(), sig, body);
                }
            }
            if is_test {
                self.test_names
    ...
    fn check_trait_item(&mut self, cx: &LateContext<'tcx>, trait_item: &'tcx TraitItem<'tcx>) {
        if let TraitItemKind::Fn(sig, TraitFn::Provided(body_id)) = &trait_item.kind {
            ...
            if !name.starts_with("rvs_") {
                self.rvs_collect_callgraph_for_item_M(cx, trait_item.hir_id(), sig, body);
            }
        }
    }
}
