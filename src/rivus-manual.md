...
注意：推断输入为分发 seed + 项目 `seed` + `std` + `ext`（不含 `deps`/`suppress`：deps 是上一次生成的结果会自我污染，suppress 修正不传播进生成的 deps 记录——间接调用方的 caps 按未修正知识生成，修正只在运行时层级生效）。输入层（含分发 seed）已记录的路径不重复生成——ext 中的手工标定保留在 ext 原处，由运行时层级直接读取。写出前若输出含 incomplete 记录，命令会输出一行汇总警告（incomplete 计数与手工标定指引）；生成层记录的 incomplete 不出现在 `cargo rivus check` 中。命令不会允许输出覆盖 active `caps/` 中的 `std`、`seed`、`suppress`、`ext`。首次运行时允许 `caps/` 不存在，使用分发 seed 推断并创建 `-o` 指定输出的父目录。同一项目的 `infer-std` 和 `infer-capsmap` 通过项目根目录下持久存在的 `.rivus-caps.lock` 串行化；进程内 registry 排斥同进程线程，POSIX record lock 排斥其他进程且不会被 fork 后尚未 exec 的子进程继承。
...
4. **遇到 incomplete caps knowledge warning 时**：warning 责任域按命令划分——`check` 只报告 workspace 分析自身的 incomplete 根因（artifact 不完整、bodyless 无 exact contract、不稳定 trait vote、ghost callee），caps 层记录自身的 incomplete/unknown 属于生成层，**不在 `check` 中警告**；`infer-std` / `infer-capsmap` 在生成时各自输出本层 incomplete 记录数的汇总警告。调用检查只使用已知能力下界，不能把记录当作 pure。check 的 warning 按"每个知识根因一条"产出——数量等于 workspace 根因数，不随调用点数量增长；每条 warning 携带 `root kind` 与针对该成因的修复指引；锚定根因的第一个精确调用点，直接调用方只出现在 details 中（传递性影响不逐个列出），`cargo rivus why` 查询根因本身。生成层 incomplete 的修复：std 记录写 explicit complete 记录并在记录上方用 `#` 注释写明依据（来源、版本、验证方式），依赖记录在 `caps/ext` 同样手工标定并写注释。Rivus 维护者可把 std 标定合入分发 seed 惠及所有消费者；普通项目把 std 标定写入项目自己的 `caps/seed` 覆盖层即可。没有逐项证明完整能力前，不得把记录标为 complete
