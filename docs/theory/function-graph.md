...
- `check` 的父进程在启动两个 Cargo 阶段前加载一次项目 caps 快照；第一阶段是 lint-bearing 采集编译——收集工作区函数图并执行直接 lint，其非零退出（编译错误或 deny 级直接 lint）立即终止命令；第二阶段只把合并图诊断（契约、调用合规、unknown/incomplete 与全项目 untested 选择）回放到源码 span，不重新解析 caps，最终离线能力分析必须复用命令开始时的同一份快照
